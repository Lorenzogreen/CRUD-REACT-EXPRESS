...
import Nouveauabsence  from './Nouveauabsence';
...
const Abscence = ()=>{
    ...
    const [ absenceAjouter, setabsenceAjouter ] = useState({})
    ...
    const recupererListe = _=>{
        axios.get('http://localhost:3001/personnel/lister')
        .then(reponse =>{
    ...
    const personnelAfficher = personnel.filter(item =>
        item.rf_absence.toString().toLowerCase().includes(filtre.toString().toLowerCase()) ||
        ...
        item.Type.toString().toLowerCase().includes(filtre.toString().toLowerCase()) 
        )
    ...
    const handleDelete = _=>{
        axios.delete(`http://localhost:3001/personnel/suppr/${personnelEffacer}`)
        .then(reponse =>{
    ...
    return(
        <div className = "container container-fce-page">
            ...
            <Nouveauabsence  isAjout = { isAjout } cacherAjout = { cacherAjout } donnee = { absenceAjouter } />
                            ...
                            personnelAfficher.map((personnel, key) =>
                                <tr key = { key }>
                                    ...
                                    <td>
                                        ...
                                        <button title = "supprimer" onClick = { ()=>afficherSuppression(personnel.rf_absence)}><img src = {del} /></button>
                                    </td>
                                </tr>
                            )
                        }
